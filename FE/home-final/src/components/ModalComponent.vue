    ...
    <div v-if="isVisible" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
          <h3 class="text-lg leading-6 font-medium text-gray-900">{{ apartmentName }}</h3>
          <div class="mt-2 px-7 py-3">
            <p class="text-sm text-gray-500">평수별 거래 정보</p>
            <div class="mb-4">
            ...
            <ul>
              <li v-for="transaction in displayedTransactions" :key="transaction.id" class="border-b py-2">
                <p>{{ transaction.apartmentName }} - {{ transaction.exclusiveArea }}㎡</p>
      ...
      const showCharts = async () => {
        ...
        try {
          const [apartmentResponse, regionResponse] = await Promise.all([
            ...
            getRentSalesByRegionCodeAndYears(props.regionCode, years) // 수정된 부분
          ]);
