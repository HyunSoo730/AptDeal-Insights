...
const searchApartments = () => {
  if (selectedDong.value) {
    router.push({ name: 'apartmentMap', params: { dongCode: selectedDong.value } });
  }
};
</script>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
      <div class="lg:col-span-2">
        <div class="bg-white shadow-md rounded-lg p-6 mb-8">
          ...
          <div class="mb-4">
            <input type="text" placeholder="아파트, 지역 등을 입력하세요" class="w-full p-3 rounded-t border border-gray-400 focus:outline-none focus:border-blue-500 text-lg">
          </div>
          <div class="mb-4">
            <select v-model="selectedCity" class="w-full p-3 rounded-t border border-gray-400 focus:outline-none focus:border-blue-500 text-lg">
              <option value="" disabled>시/도 선택</option>
              <option v-for="city in cities" :key="city.code" :value="city.code">{{ city.name }}</option>
            </select>
          </div>
          <div class="mb-4">
            <select v-model="selectedGu" class="w-full p-3 border border-gray-400 focus:outline-none focus:border-blue-500 text-lg">
              <option value="" disabled>구/군 선택</option>
              <option v-for="gu in filteredGus" :key="gu.code" :value="gu.code">{{ gu.name }}</option>
            </select>
          </div>
          <div class="mb-6">
            <select v-model="selectedDong" class="w-full p-3 rounded-b border border-gray-400 focus:outline-none focus:border-blue-500 text-lg">
              <option value="" disabled>동 선택</option>
              <option v-for="dong in filteredDongs" :key="dong.code" :value="dong.code">{{ dong.name }}</option>
            </select>
          </div>
          <button @click="searchApartments" class="bg-blue-500 text-white px-6 py-3 rounded-lg text-lg w-full">검색</button>
        </div>
